...
export default function Register() {
  ...
  const { register, loginWithGoogle, loginWithPhone, verifyPhoneOtp } = useAuth();
  ...
  async function handleGoogleLogin() {
    try {
      ...
      setError('Gagal mendaftar melalui Google.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative">
      <div className="absolute top-8 left-4 md:left-8">
        <BackButton to="/login" label="Kembali ke Log Masuk" />
      </div>
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="text-center text-3xl font-extrabold text-slate-900 dark:text-white">
          {authMode === 'otp' ? 'Sahkan Kod SMS' : 'Daftar Akaun Baru'}
        </h2>
        ...
        <div className="glass-card py-8 px-4 shadow sm:rounded-2xl sm:px-10 border border-slate-200 dark:border-slate-800">
          ...
          {authMode === 'email' && (
            <form className="space-y-6" onSubmit={handleEmailSubmit}>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Sahkan Kata Laluan</label>
                <div className="mt-1 relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <Lock className="text-slate-400" size={18} />
                  </div>
            ...
            <div className="mt-6">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-2 bg-white dark:bg-slate-900 text-slate-500">Atau daftar dengan</span>
                </div>
              </div>
              ...
              <div className="mt-6 grid grid-cols-1 gap-3">
                ...
                {authMode === 'email' ? (
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center px-4 py-3 border border-slate-300 dark:border-slate-700 rounded-xl shadow-sm text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all"
                  >
                    <Mail className="w-4 h-4 mr-2" /> Daftar dengan Emel
                  </button>
                )}
              </div>
            </div>
